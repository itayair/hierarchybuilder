...
def combine_nodes_by_umls_synonymous_spans_dfs_helper(dict_span_to_object, np_object, visited,
    ...
    if synonyms:
        ...
        if equivalent_object_lst:
            ...
            combine_spans_utils.combine_nodes_lst(equivalent_object_lst, dict_span_to_object, dict_object_to_global_label,
                                                  global_dict_label_to_object, combined_nodes_lst)
            for node in combined_nodes_lst:
                visited.add(node)

    children_lst = np_object.children.copy()
    for child in children_lst:
        combine_nodes_by_umls_synonymous_spans_dfs_helper(dict_span_to_object, child, visited,
                                                        dict_object_to_global_label,
                                                        global_dict_label_to_object, span_to_vector)


def combine_nodes_by_umls_synonymous_spans(dict_span_to_object, dict_object_to_global_label, global_dict_label_to_object,
    ...
    for topic_object in topic_object_lst:
        if topic_object in visited:
            ...
            continue
        combine_nodes_by_umls_synonymous_spans_dfs_helper(dict_span_to_object, topic_object, visited,
                                                        dict_object_to_global_label,
                                                        global_dict_label_to_object, span_to_vector)